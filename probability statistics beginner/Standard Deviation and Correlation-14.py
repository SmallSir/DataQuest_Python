...
difference = [i - values_median for i in values]
median_difference_sum = sum(difference)
...
def calc_column_deviation(cols):
    means = cols.mean()
    sum = 0
    for col in cols:
        sum += (col - means) ** 2
    return (sum / len(cols)) ** (1/2)
...
point = np.arange(-10,10,0.1)
probabilities = norm.pdf(point,0,2)
plt.plot(point,probabilities)
plt.show()
...
def covariance(x,y):
    ...
    codeviates = [x_diffs[i] * y_diffs[i] for i in range(len(x_diffs))]
...
print(nba_stats['ast'].var())
print(nba_stats['ast'].std())